...
$s^+(G)>n$.  The sharp block-additive doubly nonnegative bound reduces
...
trees attached at arbitrary vertices are allowed.  Fully shared residuals are
closed by deleting a private cyclic vertex or by partitioning a hostile central
cycle into three triangular unicyclic territories.
\end{abstract}
...
have cycle multisets $\{Q\}$ and $\{T_1,T_2\}$, respectively.  Every off-core
branch rooted at $z$ goes with the $Q$ side.  Hence their surplus sum is
...
connector, and $Q$ on one side, together with every off-core branch rooted at
$x_1$.  Put $P-x_1$ and $T_2$ on
...
\section{Fully shared residuals}

Let $I$ denote the bipartite incidence tree whose nodes are the four cyclic
blocks and the cut vertices belonging to at least two of them.  If $c$ is the
number of cut nodes, then $|E(I)|=c+3$ and
\begin{equation}\label{eq:incidence-excess}
 \sum_{x\text{ cut node}}(\deg_I(x)-1)=3.
\end{equation}

\begin{proposition}[Fully shared $\{3,3,5,5\}$]\label{prop:shared3355}
If the four cyclic blocks have lengths $3,3,5,5$ and form one shared-cut
cluster, then $\spE(G)>n$.
\end{proposition}

\begin{proof}
Suppose first that a triangle is a leaf cycle node of $I$, with unique shared
cut $x$.  Choose a private vertex $v$ of that triangle, let $F$ consist of $v$
and every off-cycle tree branch rooted there, and put the remaining vertices in
$H$.  Then $F$ is a tree, so $\sig(F)=-1$.  The broken triangle leaves an edge
at $x$, while the other triangle and two pentagons remain connected in one
shared-cut cluster.  Thus \eqref{eq:pack5} gives
\[
 \sig(G)\geq\sig(F)+\sig(H)>-1+6-2\sqrt5=5-2\sqrt5>0.
\]

Suppose neither triangle is a leaf.  Every cut node has degree at least two,
the two triangle nodes have degree at least two, and the pentagon nodes have
degree at least one.  Since $|E(I)|=c+3$, degree counting gives
$c+3\geq2c$ and $c+3\geq6$, hence $c=3$.  Equality holds throughout: every
cut node has degree two, both triangles have degree two, and both pentagons are
leaves.  Therefore $I$ is the alternating path
\[
 P_1-x-T_1-y-T_2-z-P_2.
\]
Choose a private vertex $v$ of $P_1$ and define the tree $F$ as above.  The
remainder $H$ is connected, has cycles $T_1,T_2,P_2$, and its triangles share
$y$.  Equation~\eqref{eq:pack4} gives
\[
 \sig(G)>-1+(2-\delta_5)=1-\delta_5=3-\sqrt5>0.
\]
The two incidence alternatives are exhaustive.
\end{proof}

\begin{lemma}[Three-petal partition]\label{lem:petals}
Suppose a cycle $Q=C_q$ carries three pairwise disjoint triangular blocks at
three distinct vertices.  Then the vertices can be partitioned into three
connected induced triangular unicyclic cacti.
\end{lemma}

\begin{proof}
List the three attachment vertices in cyclic order.  Partition $V(Q)$ into
three nonempty consecutive cyclic intervals, each containing exactly one
attachment vertex; equivalently, choose the three boundary edges between
successive intervals.  If two attachments are adjacent, their common edge may
be a boundary edge, and for $q=3$ all intervals are singletons.  Every interval induces
a proper path, never the whole cycle.  Add to each interval its attached
triangle.  Every component outside the four-cycle core is a tree with a unique
core attachment, since all four cyclic blocks have already been listed;
assign it wholly to the part containing that attachment.  Each resulting
induced graph is connected and its unique cycle is its triangle.
\end{proof}

\begin{proposition}[Fully shared $\{3,3,3,q\}$]\label{prop:shared333q}
If the four cyclic blocks have lengths $3,3,3,q$, $q\geq3$, and form one
shared-cut cluster, then $\spE(G)>n$.
\end{proposition}

\begin{proof}
When $q=3$, all four blocks are interchangeable; label them so that any chosen
intersecting pair is among the three designated triangles.  Suppose first that
two designated triangles share a cut.  Let $k$ be the number of shared cut
vertices on $Q$.  The $k$ cuts contribute at least $k$ to
\eqref{eq:incidence-excess}; the intersecting pair contributes one further
unit, whether its cut lies off $Q$ or is a degree-at-least-three cut on $Q$.
Hence $k+1\leq3$, so $k\leq2$.  Choose a private vertex $v$ of $Q$, let $F$
be $v$ with all off-$Q$ branches rooted there, and let $H$ be the complement.
Then $F$ is a tree and $Q-v$ is a path containing every shared cut of $Q$, so
$H$ is connected.  Its only cycles are three triangles, including an
intersecting pair, and therefore its cycle packing number is at most two.
Equation~\eqref{eq:favorable} gives $\sig(H)>2$, whence
\[
 \sig(G)\geq\sig(F)+\sig(H)>-1+2=1.
\]

If no two designated triangles meet, connectedness of $I$ forces every
triangle to meet $Q$.  No triangle can meet $Q$ twice, since that would create
a cycle in $I$, and no two can use the same cut.  Thus they are exactly the
three petals of Lemma~\ref{lem:petals}.  Each resulting triangular unicyclic
territory has strictly positive surplus by \eqref{eq:pack1}; summing with
\eqref{eq:super} proves $\sig(G)>0$.
\end{proof}

\section{Main theorem}

\begin{theorem}\label{thm:main}
Let $G$ be a connected tetracyclic cactus on $n$ vertices.  Then
\[
 \boxed{\spE(G)>n}.
\]
\end{theorem}
...
\eqref{eq:residual}.  If the shared-cut graph is disconnected,
Propositions~\ref{prop:333q} and \ref{prop:3355} settle the two residual
families.  If it is connected, Propositions~\ref{prop:shared333q} and
\ref{prop:shared3355} settle them.  These alternatives exhaust every cactus
incidence.  Every construction uses vertex-disjoint induced territories, so
crossing edges are discarded only through \eqref{eq:super}; no edge
monotonicity or tree relocation is used.
\end{proof}

\section{Conclusion}

Since a connected tetracyclic graph has $m=n+3$, the strict theorem proves the
conclusion of the Akbari--Kumar--Mohar--Pragada--Zhang conjecture
\cite[Conjecture~1.2]{AKMPZ} for every tetracyclic cactus, with arbitrary bridge
blocks and arbitrary attached trees.  A previously attempted coefficientwise
phase certificate is unnecessary: the incidence-tree sacrifice proof above
bypasses the fully shared core calculation entirely.
