#!/usr/bin/env python3
"""Exact fail-closed order-six rank-five tetrahedral census experiment.

The artifact is experimental only: it freezes the complete finite source and
the residual canonical-plus-coordinate search ledger, but no theorem claim.
"""
...
ARTIFACT = HERE / "order6-tetra-census.json"
PAIRS = tuple(itertools.combinations(range(6), 2))
EXPECTED_KERNELS = 38
EXPECTED_PHYSICAL = 23208
EXPECTED_ORBITS = 12810
EXPECTED_RESIDUALS = 1498
PATHS_PER_KERNEL = 10
...
def color_patterns():
    # Fix vertex zero to color zero. Every coloring is equivalent under a
    # global color permutation to one in this deterministic reduced list.
    return tuple((0,) + suffix for suffix in itertools.product(range(4), repeat=5))


COLORINGS = color_patterns()
...
def minimum_tetra_cost(kernel, row):
    admissible = (tetra_cost(kernel, row, coloring) for coloring in COLORINGS)
    return min(cost for cost in admissible if cost is not None)
...
def source_kernels():
    ...
    kernels = tuple(tuple(record["code"]) for record in payload["kernels"]
                    if record["n"] == 6)
    require(len(kernels) == EXPECTED_KERNELS, "order-six kernel count changed")
    return kernels
...
def regenerate():
    ledgers = []
    residuals = []
    for kernel_number, kernel in enumerate(source_kernels(), 41):
    ...
    return {
        "schema": "rank-five-order-six-tetra-census-experiment-v1",
...
def main():
    ...
    print("order-six rank-five tetra census: exact audit passed")
