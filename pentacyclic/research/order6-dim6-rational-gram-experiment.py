#!/usr/bin/env python3
"""Deterministic dimension-six rational Gram experiment for order six.
...
OUTPUT = HERE / "order6-dim6-rational-gram-results.json"
...
def path_ledger(kernel, row, frontier=None):
    ...
    require(len(paths) == 10, "order-six rank-five path count changed")
    if frontier is not None:
        require(type(frontier) is int and 0 <= frontier < 10, "invalid frontier")
...
def load_census():
    spec = importlib.util.spec_from_file_location("order6_census", CENSUS_SCRIPT)
...
def parse_frontiers(text):
    if text == "all":
        return (None, *range(10))
    result = []
    for value in text.split(","):
        result.append(None if value == "canonical" else int(value))
    require(result and all(value is None or 0 <= value < 10 for value in result),
            "frontiers must be all, canonical, or coordinates 0..9")
    return tuple(result)
...
def run(args):
    ...
    return {
        "schema": "rank-five-order-six-dim6-rational-gram-experiment-v1",
...
def main():
    ...
    verify(payload)
    print("order-six dimension-six rational Gram experiment complete")
