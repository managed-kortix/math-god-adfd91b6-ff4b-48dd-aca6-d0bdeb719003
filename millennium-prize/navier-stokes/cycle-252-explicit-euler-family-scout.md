...
4. an exact smooth periodic, `z`-independent 2D3C family with a steady
   two-dimensional carrier can vary its velocity `L^3`, but its ratio at any
   two times is at most `2^(1/6)`, far below two.
...
explicit steady-shear transport families. Use product Lebesgue measure on
`T^3=T^2 x T_z`, with either every factor normalized to mass one or no factor
normalized. Define `A` and `B` as the three-dimensional norms of the
`z`-independent lifts of `v` and `w`; in the unnormalized convention both
contain the same factor `|T_z|^(1/3)`. Thus no volume factor is missing. The
scalar transport preserves `B=||w(t)||_3`, while `A=||v||_3` is fixed.
Minkowski has exponent `3/2`, applied to the squared magnitudes, and gives
...
interior maximum `r=1`. More precisely, it is the best constant obtainable
from only the two separate `L^3` norms. On a probability space, the measurable
static configurations

\[
 |v|=1_E,\qquad |w(s)|=1_{E^c},\qquad |w(t)|=1_E,
 \qquad |E|=1/2,                                      \tag{252.10a}
\]

attain `2^(1/6)`: the lower pointwise inequality is an equality at time `s`,
Minkowski is an equality at time `t`, and `A=B`. Smooth approximations show
that no smaller constant follows merely from equimeasurability of the passive
scalar. This example is not asserted to be generated by a smooth steady Euler
carrier. Consequently (252.10) is a rigorous uniform no-go bound, but the
optimal constant within the narrower dynamical class of smooth steady-Euler
transport is not identified here and could be smaller. In particular, if
`C_dyn` denotes the supremum over that class, the proved statement is only
`C_dyn<=2^(1/6)`; no equality or stronger universal constant is established.
This closes every such proposal whose target is a factor of two, even though
the complete velocity `L^3` need not be constant.

The scope is exactly the `z`-independent 2D3C ansatz (252.6). Constant vertical
means and horizontal harmonic/mean components are already included in `w` and
in the assumed-steady full carrier `v`; transport preserves the former's full
distribution, and neither changes the estimate. If the breaker is required to
have zero total mean, one simply imposes the separately conserved conditions
`\int_{T^2}v=0` and `\int_{T^2}w=0`.
There is also no hidden `z`-dependent passive scalar in this ansatz:
incompressibility with a `z`-independent divergence-free horizontal carrier
forces `partial_z w=0`. Allowing `z`-dependent horizontal velocity leaves the
2D3C class and is not covered by (252.10).
...
| steady-carrier, `z`-independent 2D3C transport | yes | yes | yes | possible | ratio at most `2^(1/6)`; dynamical optimum not claimed |
