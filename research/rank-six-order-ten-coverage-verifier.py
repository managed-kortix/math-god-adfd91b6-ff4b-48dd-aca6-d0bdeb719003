#!/usr/bin/env python3
"""Fail-closed completion gate for the order-ten rank-six proof architecture."""
...
AUDITOR_PATH = EXPERIMENTS / "rank6_order10_pack_auditor.py"
...
EXPECTED_FRONTIERS_PER_RESIDUAL = 16
EXPECTED_SYMBOLIC_DECOMPOSITIONS = 178
EXPECTED_SYMBOLIC_TARGETS = 692
...
def completion_payload(manifest_path, exact=True):
    ...
    auditor = load_module("rank6_order10_completion_pack_auditor", AUDITOR_PATH)
    report, complete = auditor.audit(manifest_path, exact=exact)

    require(report["census"] == EXPECTED_CENSUS, "order-ten census changed")
    ...
    return {
        "schema": "rank-six-order-ten-master-coverage-v1",
