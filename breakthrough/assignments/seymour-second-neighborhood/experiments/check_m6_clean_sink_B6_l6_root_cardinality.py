#!/usr/bin/env python3
"""Producer-independent structural and semantic audit for clean B6-l6."""
...
def reconstruct():
    ...
    global_count = add_threshold(names, clauses, highs, "clean_B6_l6_global_high")
    edge_count = add_threshold(names, clauses, edges, "clean_B6_l6_AB_edges")
    rhs_count = add_threshold(names, clauses, holes + high_a, "clean_B6_l6_A_holes_high")
...
def manifest_payload(members, names, clauses, delta):
    ...
    lines = [MANIFEST_FORMAT, "scope\texact-clean-B6-l6-parent-group", "group\tB6-l6",
             "parents\t220", "branch\tB6", "lambda\t6",
...
def expected_metadata(manifest, delta):
    return [("format", FORMAT), ("manifest-format", MANIFEST_FORMAT),
            ...
            ("manifest-sha256", hashlib.sha256(manifest).hexdigest()), ("group", "B6-l6"),
...
def check(path):
    ...
    data = path.read_bytes()
    print(f"PASS parents=220 vars={len(names)} clauses={len(clauses)} sha256={hashlib.sha256(data).hexdigest()}")
...
def check_manifest_and_hashes(regenerate=True):
    ...
    if fields[:4] != ["B6-l6", "220", str(len(names)), str(len(clauses))] or not fields[4].isdigit() or \
            re.fullmatch(r"[0-9a-f]{64}", fields[5]) is None:
        raise RuntimeError("hash ledger row changed")
    if regenerate:
        import tempfile
        with tempfile.TemporaryDirectory(prefix="clean-B6-l6-check-", dir=HERE.parent) as directory:
    ...
    print(f"PASS manifest_sha256={hashlib.sha256(manifest).hexdigest()} parents=220")
