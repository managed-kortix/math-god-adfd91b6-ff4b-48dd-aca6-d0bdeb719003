#!/usr/bin/env python3
"""Strict ledger verification and fresh replay of all 53 B7-l5 LRATs."""
...
import check_m6_b7_l5_profile_root_cardinality as structural
...
LEDGER_CANONICAL_SHA256 = "4b387244695549a524ed62e864c6b0f6afbde6fda6e10906f127dc9dbb42649a"
...
LIMIT = 250_000_000
TOTALS = {"cnf-bytes": 573429055, "lrat-bytes": 1004531865, "xz-bytes": 121021160}
COLUMNS = ("position", "key", "state-key", "t", "parents", "variables", "clauses",
...
BOUND_PATHS = {
    ...
    "producer": HERE / "m6_b7_l5_profile_root_cardinality.py",
    "checker": HERE / "check_m6_b7_l5_profile_root_cardinality.py",
    "scout-producer": HERE / "m6_b7_l5_profile_root_cardinality_scout.py",
    "certificate-producer": HERE / "certify_m6_b7_l5_profile_root_cardinality.py",
    "hostile-tests": HERE / "test_m6_b7_l5_profile_root_cardinality.py",
    "composition-verifier": HERE / "verify_m6_b7_l5_profile_root_cardinality_closure.py",
}
...
def load_ledger(path=LEDGER):
    ...
    for line in lines[1:]:
        ...
        if fields[0] == "columns":
            ...
            rows.append(row)
    required = {
        "generated-utc": "2026-09-02", "scope": "all-53-clean-B7-l5-profile-root-cardinality",
        "profiles": "53", "parents": "322", "scout-unsat": "53", "certified-profiles": "53",
    ...
    if columns is None or len(rows) != 53 or set(metadata) != expected or any(
    ...
    if tuple((int(row["position"]), row["key"]) for row in rows) != tuple(
            (i, f"p{i:02d}") for i in range(53)) or any(
            sum(int(row[key]) for row in rows) != value for key, value in TOTALS.items()) or \
...
def verify_bindings(metadata, checker=None):
    ...
    if scout.get("profiles") != 53 or scout.get("unsat") != 53 or scout.get("sat") or scout.get("timeout"):
...
def artifact_paths(rows):
    ...
    if len(expected) != 53 or actual != expected or any(
            not re.fullmatch(r"certificates/m6-b7-l5-profile-root-cardinality-profile-[0-9]{2}\.lrat\.xz", path)
...
def verify_payloads(rows, checker=None):
    ...
    with tempfile.TemporaryDirectory(prefix="b7-l5-root-verify-") as directory:
        work = Path(directory)
        for count, row in enumerate(rows, 1):
            ...
            expected_description = {
                "key": profile[0], "state-key": profile[2], "t": str(profile[4]),
            ...
            if checker is not None:
                ...
                lrat_path.unlink()
            cnf_path.unlink()
            print(f"PASS {count:02d}/53 profile={position:02d}")
...
def main():
    ...
    print(f"PASS profiles=53 parents=322 total_xz_bytes={TOTALS['xz-bytes']} limit_exclusive={LIMIT}")
