#!/usr/bin/env python3
"""Run a one-second scout over the exact low-C inaccessible-pair cover."""
...
def main():
    ...
    if str(args.solver) != SOLVER_PATH or args.seconds != 1:
        parser.error(f"pinned scout requires --solver {SOLVER_PATH} --seconds 1")
    ...
    rows = []
    with tempfile.TemporaryDirectory(prefix="inaccessible-pair-scout-", dir=producer.HERE.parent) as directory:
        ...
        for ordinal, child in enumerate(children):
            ...
            if digest != hashes[child[0]]:
                raise RuntimeError(f"child {ordinal:03d} hash differs before scout")
            ...
            if status not in ("UNSAT", "TIMEOUT"):
                raise RuntimeError(f"unexpected scout result {status} for child {ordinal}")
            rows.append({"child": ordinal, "key": child[0], "profile": child[1],
                         "compatible_parents": len(child[5]), "status": status, "seconds": elapsed,
                          "cnf_sha256": digest})
            print(f"child={ordinal:03d} key={child[0]} status={status} seconds={elapsed}")
    status_sequence = "".join({"UNSAT": "U", "TIMEOUT": "T"}[row["status"]] for row in rows)
    payload = {"schema": f"{producer.PREFIX}-scout-v1", "seconds_per_child": args.seconds,
