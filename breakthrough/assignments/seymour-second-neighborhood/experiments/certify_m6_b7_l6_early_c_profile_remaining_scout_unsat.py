#!/usr/bin/env python3
"""Create checked LRATs for the five remaining frozen early-profile cells."""
...
def main():
    ...
    try:
        ...
        if tuple(ordinal for ordinal in SCOPE if scout.get(ordinal) == "UNSAT") != SCOPE:
            raise RuntimeError("scope is not exactly inside the frozen SCOUT-UNSAT set")
        records, new_xz = [], 0
        for position, ordinal in enumerate(SCOPE, 1):
            ...
            new_xz += xz_id[0]
            if PRIOR_XZ_BYTES + new_xz >= LIMIT:
                raise RuntimeError(
                    f"combined compressed total {PRIOR_XZ_BYTES + new_xz} is not below {LIMIT}"
                )
            ...
            lrat_path.unlink()
            print(
                f"PASS {position:02d}/05 orbit={ordinal:02d} xz={xz_id[0]} "
                f"combined={PRIOR_XZ_BYTES + new_xz}", flush=True,
            )
        ...
        print(
            f"PASS orbits=5 new_xz_bytes={new_xz} "
            f"combined_xz_bytes={PRIOR_XZ_BYTES + new_xz} rows={rows}"
        )
