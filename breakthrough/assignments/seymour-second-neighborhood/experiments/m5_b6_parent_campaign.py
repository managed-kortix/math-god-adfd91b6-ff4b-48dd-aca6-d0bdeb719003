...
def main():
 p=argparse.ArgumentParser();p.add_argument("--root",required=True);p.add_argument("--cadical",required=True);p.add_argument("--checker",required=True);p.add_argument("--jobs",type=int,default=4);a=p.parse_args();assert hashlib.sha256(payload()).hexdigest()==EXPECTED
