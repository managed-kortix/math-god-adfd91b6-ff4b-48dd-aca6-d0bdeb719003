#!/usr/bin/env python3
"""Add the exact no-gain child to every frozen hard witness leaf."""
...
def dimensions(leaf):
    ...
    return variables, clauses + len(no_gain_paths(leaf))


def build_leaf(leaf):
    cnf, selectors = source.build_leaf(leaf)
    for witness, midpoint, deleted in no_gain_paths(leaf):
...
def manifest_payload(leaves):
    ...
    for ordinal, leaf in enumerate(leaves):
        ...
        lines.append(f"{ordinal:03d}\t{leaf[0]}\t{leaf[1]:02d}\t{leaf[2][0]}\t"
                     ...
                     f"{len(no_gain_paths(leaf))}\t{len(leaf[2][6])}\t{variables}\t{clauses}")
    return ("\n".join(lines) + "\n").encode("ascii")
...
def hash_payload(leaves, manifest, hashes=None):
    ...
    lines = [HASH_FORMAT, f"manifest-bytes\t{len(manifest)}",
             ...
             "columns\tleaf-ordinal,key,negative-path-units,parents,variables,clauses,cnf-sha256"]
    for ordinal, leaf in enumerate(leaves):
        variables, clauses = dimensions(leaf)
        lines.append(f"{ordinal:03d}\t{leaf[0]}\t{len(no_gain_paths(leaf))}\t"
...
def metadata(ordinal, leaf, manifest, selectors):
    ...
    result.extend((("leaf-ordinal", str(ordinal)), ("source-witness-key", leaf[0]),
                   ...
                   ("negative-path-unit-clauses", str(len(no_gain_paths(leaf)))),
...
def populate_hashes(leaves, manifest):
    hashes = {}
    with tempfile.TemporaryDirectory(prefix="m6-witness-no-gain-hashes-", dir=HERE.parent) as directory:
...
def main():
    ...
    if args.output:
        ...
        print(f"leaf={args.leaf:03d} key={leaf[0]} units={len(no_gain_paths(leaf))} "
