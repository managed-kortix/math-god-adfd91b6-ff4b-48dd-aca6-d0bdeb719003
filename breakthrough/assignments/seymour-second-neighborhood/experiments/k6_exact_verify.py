#!/usr/bin/env python3
"""Freshly verify every retained exact-pressure campaign proof object."""
...
def main():
 ...
 for i,k in enumerate(keys()):
  meta=os.path.join(a.root,"leaves",f"{i:04d}.json")
  with open(meta) as f:r=json.load(f)
  ...
  with tempfile.TemporaryDirectory(dir=os.path.join(a.root,"work")) as d:
   cnf=os.path.join(d,"x.cnf");proof=os.path.join(d,"x.lrat");emit(i,cnf,exact_pressure=True);assert sha(cnf)==r["cnf_sha256"]
