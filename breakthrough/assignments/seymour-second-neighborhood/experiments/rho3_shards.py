#!/usr/bin/env python3
"""Emit the three exact rho=3 order-18 CNF shards (k T-holes, k=0,1,2)."""
import argparse
...
def emit(k, output):
    ...
    rho = threshold(c, [c.var(f"a_{x}_{b}") for x in (16, 17)
                        for b in range(9, 16)], f"rho3_k{k}")
    c.add(rho[2]); c.add(-rho[3])
    ht = threshold(c, [c.var(f"h_{i}_{j}") for i in range(16)
                       for j in range(i + 1, 16)], f"ht_k{k}")
    if k == 0:
        c.add(-ht[0])
    else:
...
if __name__ == "__main__":
    ...
    p.add_argument("k", type=int, choices=(0, 1, 2))
    p.add_argument("output")
    a = p.parse_args(); emit(a.k, a.output)
