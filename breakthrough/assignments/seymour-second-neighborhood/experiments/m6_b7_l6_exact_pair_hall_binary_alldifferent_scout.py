#!/usr/bin/env python3
"""Scout all 33 certificate-relative binary all-different memberships."""
...
import m6_b7_l6_exact_pair_hall_binary_alldifferent as producer
...
def main():
    ...
    if str(args.solver) != SOLVER or args.seconds != SECONDS or args.jobs != JOBS or \
        ...
        parser.error("scout requires pinned CaDiCaL 1.7.3, 30 seconds, and eight jobs")
    ...
    def job(job_number, directory):
        ...
        for position in range(job_number, len(records), JOBS):
            ...
            cnf, selectors, universe, support = producer.build_membership(records[position])
            producer.write_membership(path, position, records[position], cnf, selectors, universe,
                                      support, manifest)
            if producer.identity(path) != hashes[position]:
                raise RuntimeError("scout CNF differs from generated hash")
            start = time.monotonic()
            result = subprocess.run(["timeout", str(SECONDS), str(args.solver), "-q",
                                     *SOLVER_OPTIONS, path], stdout=subprocess.DEVNULL,
                                    stderr=subprocess.STDOUT)
            elapsed = round(time.monotonic() - start, 3)
            status = {20: "UNSAT", 124: "TIMEOUT"}.get(result.returncode)
            if status is None:
    ...
    rows = []
    with tempfile.TemporaryDirectory(prefix="hall-binary-scout-", dir=producer.HERE.parent) as directory:
    ...
    payload = {"schema": f"{producer.PREFIX}-scout-v1", "seconds_per_membership": SECONDS,
