#!/usr/bin/env python3
"""Fresh replay of the five remaining frozen early-profile SCOUT-UNSAT LRATs."""
...
ROOT = HERE.parent
LEDGER = HERE / "m6-b7-l6-early-c-profile-remaining-scout-unsat-certificates.tsv"
FORMAT = "m6-b7-l6-early-c-profile-remaining-scout-unsat-certificates-v1"
LEDGER_CANONICAL_SHA256 = "4779d70fdee77c8a89db4880c883fdfbd98e5da825181a1221388bc6610eaff9"
...
LIMIT = 250_000_000
PRIOR_XZ_BYTES = 13_906_564
NEW_XZ_BYTES = 76_286_284
COMBINED_XZ_BYTES = 90_192_848
SCOPE = (4, 5, 31, 32, 33)
COLUMNS = (
...
BOUND_PATHS = {
    "prior-certificate-ledger": HERE / "m6-b7-l6-early-c-profile-scout-unsat-certificates.tsv",
    ...
    "certificate-producer": HERE / "certify_m6_b7_l6_early_c_profile_remaining_scout_unsat.py",
    "certificate-documentation": ROOT / "attempts/frozen-b7-l6-early-c-profile-remaining-certificates.md",
}
...
def load_ledger(path=LEDGER):
    ...
    required = {
        ...
        "scope": "frozen-B7-l6-early-C-profile-remaining-five-SCOUT-UNSAT-orbits-only",
        "base-commit": "3e176b4675a4d4676cae9eeab8399a74ef19f265",
        "scope-orbits": ",".join(f"{value:02d}" for value in SCOPE),
        "census-orbits": "60", "census-scout-unsat": "31", "certified-orbits": "5",
        "prior-certified-orbits": "26", "total-scout-unsat-certified-orbits": "31",
        ...
        "compressed-limit-bytes": str(LIMIT), "prior-xz-bytes": str(PRIOR_XZ_BYTES),
        "new-lrat-bytes": "382731330", "new-xz-bytes": str(NEW_XZ_BYTES),
        "combined-xz-bytes": str(COMBINED_XZ_BYTES),
    }
    ...
    if sum(int(row["lrat-bytes"]) for row in rows) != 382_731_330 or \
            sum(int(row["xz-bytes"]) for row in rows) != NEW_XZ_BYTES or \
            PRIOR_XZ_BYTES + NEW_XZ_BYTES != COMBINED_XZ_BYTES or COMBINED_XZ_BYTES >= LIMIT:
        raise RuntimeError("proof byte total differs or is not below 250MB")
...
def verify_bindings(metadata, checker):
    ...
    if compressor != Path("/usr/bin/xz") or \
            identity(compressor)[1] != metadata["compressor-binary-sha256"]:
        raise RuntimeError("compressor is not the pinned /usr/bin/xz")
...
def main():
    ...
    if len(orbits) != 60 or tuple(i for i in SCOPE if scout.get(i) == "UNSAT") != SCOPE:
        raise RuntimeError("scope differs from frozen 60-orbit census SCOUT-UNSAT rows")
    with tempfile.TemporaryDirectory(prefix="remaining-profile-replay-", dir=ROOT) as directory:
        work = Path(directory)
        for position, row in enumerate(rows, 1):
            ...
            print(f"PASS {position:02d}/05 orbit={ordinal:02d} xz={row['xz-bytes']}")
    print(
        f"PASS orbits=5 new_xz_bytes={NEW_XZ_BYTES} "
        f"combined_xz_bytes={COMBINED_XZ_BYTES} limit_bytes={LIMIT}"
    )
