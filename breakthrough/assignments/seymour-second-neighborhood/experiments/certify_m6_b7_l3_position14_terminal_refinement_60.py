#!/usr/bin/env python3
"""Generate, check, and compress all 60 position-14 terminal LRATs."""
...
def main():
    ...
    for ordinal, leaf in enumerate(leaves):
        ...
        cnf_path.unlink()
        lrat_path.unlink()
        print(f"PASS {ordinal + 1:02d}/60 leaf={ordinal:02d} xz={xz_id[0]} total={total_xz}", flush=True)
    with (args.stage / "rows.tsv").open("w", encoding="ascii", newline="\n") as handle:
        handle.write("columns\t" + ",".join(COLUMNS) + "\n")
        for row in rows:
            handle.write("\t".join(row) + "\n")
    print(f"PASS leaves=60 parents=1269 total_xz_bytes={total_xz}")
