#!/usr/bin/env python3
"""Independently reconstruct and audit the clean B7-l5 53-profile campaign."""
...
PREFIX = "m6-b7-l5-profile-root-cardinality"
...
def derive():
    ...
    if (len(parents), len(states), sum(len(x[2]) for x in states), len(profiles),
            sum(len(x[7]) for x in profiles)) != (322, 30, 1920, 53, 3387):
        raise RuntimeError("independent B7-l5 census differs")
...
def reconstruct(profile):
    ...
    for i in range(len(members)):
        names.append(f"b7_l5_profile_parent_{i:03d}")
    ...
    global_count = add_threshold(names, clauses, high_all, "b7_l5_root_global_high")
    edge_count = add_threshold(names, clauses, edges, "b7_l5_root_AB_edges")
    rhs_count = add_threshold(names, clauses, holes + high_a, "b7_l5_root_A_holes_high")
...
def load_manifest():
    ...
    if len(rows) != 53 or "certificate-status\tnot-started" not in lines:
...
def load_hashes(manifest):
    ...
    expected = [f"{PREFIX}-hashes-v1", f"manifest-bytes\t{len(manifest)}",
                f"manifest-sha256\t{hashlib.sha256(manifest).hexdigest()}", "profiles\t53",
                "columns\tposition,key,parents,variables,clauses,cnf-bytes,cnf-sha256"]
    if lines[:5] != expected or len(lines) != 58:
...
def check_cover(regenerate=True):
    ...
    if tuple(row.split("\t")[1] for row in rows) != tuple(f"p{i:02d}" for i in range(53)):
    ...
    if regenerate:
        with tempfile.TemporaryDirectory(prefix="b7-l5-root-check-", dir=HERE.parent) as directory:
    ...
    print(f"PASS parents=322 states=30 profiles=53 incidences=3387 manifest_sha256={hashlib.sha256(manifest).hexdigest()}")
