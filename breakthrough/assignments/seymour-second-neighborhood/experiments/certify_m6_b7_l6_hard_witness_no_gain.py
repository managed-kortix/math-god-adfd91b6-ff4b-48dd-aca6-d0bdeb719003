#!/usr/bin/env python3
"""Create checked LRATs for exactly the frozen scout-UNSAT no-gain leaves."""
...
import check_m6_b7_l6_hard_witness_no_gain as structural
...
def main():
    ...
    try:
        ...
        if len(ordinals) != 75:
            raise RuntimeError("scope is not exactly the 75 frozen scout-UNSAT leaves")
        ...
        for position, ordinal in enumerate(ordinals, 1):
            ...
            artifact_name = f"m6-b7-l6-hard-witness-no-gain-leaf-{ordinal:03d}.lrat.xz"
            ...
            compressed = subprocess.run(
                ["xz", "-3", "-c", str(lrat)], stdout=artifact.open("wb"), check=False,
            )
            ...
            records.append((
                ...
                xz_id[1], str(solve_ns), str(check_ns),
                f"certificates/{artifact_name}",
            ))
            cnf.unlink()
            lrat.unlink()
            print(f"PASS {position:02d}/75 leaf={ordinal:03d} key={leaf[0]} xz={xz_id[0]} total={total_xz}", flush=True)
        ...
        print(f"PASS leaves=75 total_xz_bytes={total_xz} rows={rows}")
