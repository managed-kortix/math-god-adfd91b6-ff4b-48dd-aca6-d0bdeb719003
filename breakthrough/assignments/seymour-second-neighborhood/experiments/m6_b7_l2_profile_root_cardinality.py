#!/usr/bin/env python3
"""Emit the clean B7-l2 4-profile root-cardinality campaign."""
...
PREFIX = "m6-b7-l2-profile-root-cardinality"
...
PARENT_COUNT = 8119
STATE_COUNT = 4
STATE_INCIDENCES = 32476
PROFILE_INCIDENCES = 32476
SOURCE_PATHS = {
...
def load_profiles():
    ...
    if len(parents) != PARENT_COUNT or len(states) != STATE_COUNT or \
        ...
        raise RuntimeError("frozen B7-l2 8119/4/32476/4/32476 census changed")
    return profiles
...
def extend(cnf):
    ...
    global_count = threshold(cnf, high_all, "b7_l2_root_global_high")
    edge_count = threshold(cnf, edges, "b7_l2_root_AB_edges")
    rhs_count = threshold(cnf, holes + high_a, "b7_l2_root_A_holes_high")
...
def build(profile):
    ...
    selectors = [cnf.var(f"b7_l2_profile_parent_{i:03d}") for i in range(len(members))]
    cnf.add(*selectors)
...
def main():
    ...
    if args.populate_hashes:
        values = []
        with tempfile.TemporaryDirectory(prefix="b7-l2-root-hashes-", dir=HERE.parent) as directory:
    ...
    print(f"PASS parents=8119 states=4 profiles=4 incidences=32476 manifest_sha256={hashlib.sha256(manifest).hexdigest()}")
