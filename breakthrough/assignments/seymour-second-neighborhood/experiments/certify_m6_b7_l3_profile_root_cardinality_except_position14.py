#!/usr/bin/env python3
"""Generate, check, and compress LRATs for B7-l3 except position 14."""
    ...
    for count, position in enumerate(POSITIONS, 1):
        ...
        try:
            solved = subprocess.run(
                [str(solver), "--lrat", "--no-binary", "-q", "--restart=false", "--phase=false",
                 "--seed=3", str(cnf_path), str(lrat_path)],
                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT, timeout=TIMEOUT_SECONDS)
        except subprocess.TimeoutExpired as error:
            raise RuntimeError(f"profile {position:02d} exceeded {TIMEOUT_SECONDS}s") from error
        ...
        cnf_path.unlink()
        lrat_path.unlink()
        print(f"PASS {count:02d}/18 position={position:02d} xz={xz_id[0]} total={total_xz}", flush=True)
    with (args.stage / "rows.tsv").open("w", encoding="ascii", newline="\n") as handle:
        handle.write("columns\t" + ",".join(COLUMNS) + "\n")
        for row in rows:
            handle.write("\t".join(row) + "\n")
    print(f"PASS profiles=18 excluded_position=14 total_xz_bytes={total_xz}")
