...
import verify_m6_b7_l4_profile_root_cardinality_certificates as verifier
...
class B7L5ProfileRootCardinalityTest(unittest.TestCase):
    ...
    def test_rejects_noncanonical_artifact_path(self):
        data = verifier.LEDGER.read_text(encoding="ascii")
        mutated = data.replace("certificates/m6-b7-l4-profile-root-cardinality-profile-00.lrat.xz",
    ...
    def test_rejects_transitive_runtime_mutation(self):
        metadata, _ = verifier.load_ledger()
        path = verifier.runtime_source_closure()[0]
        key = f"runtime-{path.name[:-3].replace('_', '-')}-sha256"
        mutated = dict(metadata)
        mutated[key] = "0" * 64
        with self.assertRaisesRegex(RuntimeError, "transitive runtime"):
            verifier.verify_bindings(mutated)
