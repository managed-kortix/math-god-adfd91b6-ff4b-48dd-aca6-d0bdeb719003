#!/usr/bin/env python3
"""Generate, check, and compress LRATs for the exact 19 two-high profiles."""
...
def main():
    ...
    try:
        ...
        for position, ordinal in enumerate(producer.SCOPE):
            ...
            cnf_path, lrat_path = args.stage / f"o{ordinal:02d}.cnf", args.stage / f"o{ordinal:02d}.lrat"
            name = f"{producer.PREFIX}-orbit-{ordinal:02d}.lrat.xz"
            artifact = artifacts / name
            built = producer.build(ordinal, orbits)
            producer.write_cnf(cnf_path, position, ordinal, orbit, *built, manifest)
            ...
            if solved.returncode != 20:
                raise RuntimeError(f"orbit {ordinal:02d} solver exit {solved.returncode}")
            ...
            cnf, _, _ = built
            rows.append((f"{position:02d}", f"{ordinal:02d}", orbit[0], orbit[2], str(orbit[4]),
                         str(len(orbit[7])), str(len(cnf.names)), str(len(cnf.clauses)), str(cnf_id[0]),
                         cnf_id[1], str(lrat_id[0]), lrat_id[1], str(xz_id[0]), xz_id[1], str(solve_ns),
                         str(check_ns), f"certificates/{name}"))
            cnf_path.unlink()
            lrat_path.unlink()
            print(f"PASS {position + 1:02d}/19 orbit={ordinal:02d} xz={xz_id[0]} total={total_xz}", flush=True)
        with (args.stage / "rows.tsv").open("w", encoding="ascii", newline="\n") as handle:
            handle.write("columns\t" + ",".join(COLUMNS) + "\n")
            for row in rows:
                handle.write("\t".join(row) + "\n")
        print(f"PASS profiles=19 total_xz_bytes={total_xz}")
