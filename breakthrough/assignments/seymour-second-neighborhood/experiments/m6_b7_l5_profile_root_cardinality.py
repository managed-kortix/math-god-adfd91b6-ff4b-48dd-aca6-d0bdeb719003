#!/usr/bin/env python3
"""Emit the clean B7-l5 53-profile root-cardinality campaign."""
...
PARENT_COUNT = 322
STATE_COUNT = 30
STATE_INCIDENCES = 1920
PROFILE_INCIDENCES = 3387
SOURCE_PATHS = {
...
def load_profiles():
    ...
    if len(parents) != PARENT_COUNT or len(states) != STATE_COUNT or \
        ...
        raise RuntimeError("frozen B7-l5 322/30/1920/53/3387 census changed")
    return profiles
...
def extend(cnf):
    ...
    global_count = threshold(cnf, high_all, "b7_l5_root_global_high")
    edge_count = threshold(cnf, edges, "b7_l5_root_AB_edges")
    rhs_count = threshold(cnf, holes + high_a, "b7_l5_root_A_holes_high")
...
def build(profile):
    ...
    selectors = [cnf.var(f"b7_l5_profile_parent_{i:03d}") for i in range(len(members))]
    cnf.add(*selectors)
...
def main():
    ...
    if args.populate_hashes:
        values = []
        with tempfile.TemporaryDirectory(prefix="b7-l5-root-hashes-", dir=HERE.parent) as directory:
