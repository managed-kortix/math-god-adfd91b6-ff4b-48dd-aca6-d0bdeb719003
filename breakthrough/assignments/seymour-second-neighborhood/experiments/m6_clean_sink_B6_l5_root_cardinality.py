#!/usr/bin/env python3
"""Emit the exact clean B6-l5 parent group with fresh root cardinalities."""
...
def extend(cnf):
    ...
    global_count = threshold(cnf, highs, "clean_B6_l5_global_high")
    edge_count = threshold(cnf, edges, "clean_B6_l5_AB_edges")
    rhs_count = threshold(cnf, holes + high_a, "clean_B6_l5_A_holes_high")
...
def build():
    ...
    delta = extend(cnf)
    if len(members) != 1024 or delta != (2013, 7899):
        raise RuntimeError("exact B6-l5 scope or fresh-counter dimensions changed")
...
def manifest_payload(members, cnf, delta):
    ...
    lines = [MANIFEST_FORMAT, "scope\texact-clean-B6-l5-parent-group",
              "group\tB6-l5", "parents\t1024", "branch\tB6", "lambda\t5",
...
def main():
    ...
    print(f"PASS parents=1024 manifest_sha256={hashlib.sha256(manifest).hexdigest()}")
