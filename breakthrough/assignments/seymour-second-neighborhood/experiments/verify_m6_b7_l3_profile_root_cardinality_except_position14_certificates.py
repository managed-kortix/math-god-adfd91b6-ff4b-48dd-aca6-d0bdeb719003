#!/usr/bin/env python3
"""Strict ledger verification and fresh replay of B7-l3 except position 14."""
...
import check_m6_b7_l3_profile_root_cardinality as structural
import m6_b7_l3_profile_root_cardinality as producer

HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
STEM = f"{producer.PREFIX}-except-position14"
...
BOUND_PATHS = {
    ...
    "producer": HERE / "m6_b7_l3_profile_root_cardinality.py",
    "checker": HERE / "check_m6_b7_l3_profile_root_cardinality.py",
    "scout-producer": HERE / "m6_b7_l3_profile_root_cardinality_scout.py",
    "certificate-producer": HERE / "certify_m6_b7_l3_profile_root_cardinality_except_position14.py",
    "hostile-tests": HERE / "test_m6_b7_l3_profile_root_cardinality.py",
}
...
def package_hash(rows):
    payload = "".join("\t".join((row["position"], row["xz-bytes"], row["xz-sha256"],
                                  row["artifact"])) + "\n" for row in rows)
...
def load_packages(rows, path=PACKAGES):
    ...
    expected = [f"{STEM}-packages-v1", "compressed-limit-bytes-exclusive\t250000000",
                "packages\t4", "profiles\t18", "excluded-position\t14",
                "artifact-set-hash-format\tsha256(position-tab-xz-bytes-tab-xz-sha256-tab-artifact-newline)",
                "columns\tpackage,positions,profiles,xz-bytes,artifact-set-sha256"]
    if data != ("\n".join(lines) + "\n").encode("ascii") or lines[:7] != expected or len(lines) != 11:
        raise RuntimeError("package ledger is not canonical")
    by_position = {int(row["position"]): row for row in rows}
    covered = []
    for package, line in enumerate(lines[7:]):
        fields = line.split("\t")
        positions = tuple(map(int, fields[1].split(",")))
        selected = [by_position[position] for position in positions]
        size = sum(int(row["xz-bytes"]) for row in selected)
        if fields[:1] != [f"{package:02d}"] or int(fields[2]) != len(positions) or \
                int(fields[3]) != size or fields[4] != package_hash(selected) or size >= LIMIT:
            raise RuntimeError("package row differs")
        covered.extend(positions)
    if tuple(covered) != POSITIONS:
...
def load_ledger(path=LEDGER):
    ...
    for line in lines[1:]:
        ...
        if fields[0] == "columns":
            ...
            rows.append(row)
    required = {
        "generated-utc": "2026-09-02", "scope": "18-clean-B7-l3-profiles-except-position14",
        "profiles": "18", "campaign-profiles": "19", "excluded-position": "14", "parents": "5016",
        "states": "16", "certified-profiles": "18", "packages": "4", "profile-producer-commit":
        "2838dbb6c8553dbf21e75c3f31e4279929106323", "seconds-per-profile": "300",
    ...
    if columns is None or len(rows) != 18 or set(metadata) != expected or any(
    ...
    if tuple((int(row["position"]), row["key"]) for row in rows) != tuple(
            (position, f"p{position:02d}") for position in POSITIONS) or any(
...
def artifact_paths(rows):
    ...
    if len(expected) != 18 or actual != expected or any("profile-14.lrat.xz" in path for path in actual):
        raise RuntimeError("artifact set differs from exact except-position14 ledger")


def verify_payloads(rows, checker=None):
    profiles = producer.load_profiles()
    manifest = producer.manifest_payload(profiles)
    with tempfile.TemporaryDirectory(prefix="b7-l3-except14-verify-", dir=ROOT) as directory:
        work = Path(directory)
        for count, row in enumerate(rows, 1):
            ...
            cnf = built[0]
            expected = {"key": profile[0], "state-key": profile[2], "t": str(profile[4]),
            ...
            print(f"PASS {count:02d}/18 position={position:02d}")
...
def main():
    ...
    print(f"PASS profiles=18 excluded_position=14 packages=4 total_xz_bytes={TOTALS['xz-bytes']}")
