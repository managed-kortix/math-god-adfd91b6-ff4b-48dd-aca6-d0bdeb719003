#!/usr/bin/env python3
"""Reproduce the canonical 40-profile B7-l4 root-cardinality scout."""
...
def main():
    ...
    rows = []
    with tempfile.TemporaryDirectory(prefix="b7-l4-root-scout-", dir=producer.HERE.parent) as directory:
        ...
        for position, profile in enumerate(profiles):
            ...
            print(f"PASS {position + 1:02d}/40 profile={profile[0]} status={status}", flush=True)
    payload = {"schema": f"{producer.PREFIX}-scout-v1", "seconds_per_profile": args.seconds,
               ...
               "solver_version": "1.7.3", "solver_options": list(OPTIONS), "profiles": 40,
