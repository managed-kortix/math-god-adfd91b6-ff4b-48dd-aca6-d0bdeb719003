#!/usr/bin/env python3
"""Generate, check, and compress LRATs for all 53 B7-l5 profiles."""
...
def main():
    ...
    try:
        ...
        for position, profile in enumerate(profiles):
            cnf_path, lrat_path = args.stage / f"p{position:02d}.cnf", args.stage / f"p{position:02d}.lrat"
            ...
            rows.append((f"{position:02d}", profile[0], profile[2], str(profile[4]), str(len(profile[7])),
                         str(len(cnf.names)), str(len(cnf.clauses)), str(cnf_id[0]), cnf_id[1], str(lrat_id[0]),
                         lrat_id[1], str(xz_id[0]), xz_id[1], str(solve_ns), str(check_ns),
                         f"certificates/{name}"))
            cnf_path.unlink()
            lrat_path.unlink()
            print(f"PASS {position + 1:02d}/53 profile={profile[0]} xz={xz_id[0]} total={total_xz}", flush=True)
        with (args.stage / "rows.tsv").open("w", encoding="ascii", newline="\n") as handle:
            handle.write("columns\t" + ",".join(COLUMNS) + "\n")
            for row in rows:
                handle.write("\t".join(row) + "\n")
        print(f"PASS profiles=53 total_xz_bytes={total_xz}")
