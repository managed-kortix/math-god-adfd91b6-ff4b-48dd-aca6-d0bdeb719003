...
def main():
 p=argparse.ArgumentParser();p.add_argument("--cadical",required=True);p.add_argument("--seconds",type=int,default=5);p.add_argument("--output",required=True);a=p.parse_args()
 hard=hard_indices();indices=[i for i,k in enumerate(keys()) if parent(k) in hard];rows=[]
 for n,i in enumerate(indices):
  with tempfile.TemporaryDirectory() as d:
   cnf=os.path.join(d,"x.cnf");emit(i,cnf,packet_pressure=True);start=time.monotonic();r=subprocess.run(["timeout",str(a.seconds),a.cadical,"-q",cnf],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);elapsed=round(time.monotonic()-start,3)
