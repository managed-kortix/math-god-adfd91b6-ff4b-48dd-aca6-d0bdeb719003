#!/usr/bin/env python3
"""Strictly regenerate and replay the frozen clean B6-l5 grouped LRAT."""
...
import check_m6_clean_sink_B6_l5_root_cardinality as structural
import m6_clean_sink_B6_l5_root_cardinality as producer

HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
LEDGER = HERE / f"{producer.PREFIX}-certificate.tsv"
FORMAT = f"{producer.PREFIX}-certificate-v1"
LEDGER_CANONICAL_SHA256 = "702792d595eddba3d42263100b505dee909013de84ddd9af4435ab0a460add41"
...
BOUND_PATHS = {
    ...
    "producer": HERE / "m6_clean_sink_B6_l5_root_cardinality.py",
    "checker": HERE / "check_m6_clean_sink_B6_l5_root_cardinality.py",
    "scout-producer": HERE / "m6_clean_sink_B6_l5_root_cardinality_scout.py",
    "certificate-producer": HERE / "certify_m6_clean_sink_B6_l5_root_cardinality.py",
    "hostile-tests": HERE / "test_m6_clean_sink_B6_l5_root_cardinality.py",
...
def load_ledger(path=LEDGER):
    ...
    required = {
        ...
        "scope": "exact-clean-B6-l5-parent-group-root-cardinality",
        "groups": "1", "parents": "1024", "certified-groups": "1",
    ...
    if (row["group"], row["parents"], row["variables"], row["clauses"]) != (
            producer.GROUP, "1024", "26653", "307308") or int(row["xz-bytes"]) >= LIMIT:
...
def main():
    ...
    if not args.no_replay:
        ...
        with tempfile.TemporaryDirectory(prefix="clean-B6-l5-root-replay-", dir=ROOT) as directory:
            work = Path(directory)
    ...
    print(f"PASS group={producer.GROUP} parents=1024 xz_bytes={row['xz-bytes']} limit_exclusive={LIMIT}")
