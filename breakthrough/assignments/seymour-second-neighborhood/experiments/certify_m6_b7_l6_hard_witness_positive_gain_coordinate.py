#!/usr/bin/env python3
"""Create checked LRATs for exactly eight frozen coordinate-scout UNSAT leaves."""
...
import check_m6_b7_l6_hard_witness_positive_gain_coordinate as structural
...
def main():
    ...
    try:
        ...
        if ordinals != ORDINALS:
            raise RuntimeError("scope is not exactly frozen coordinate scout-UNSAT leaves")
        ...
        for position, ordinal in enumerate(ordinals, 1):
            ...
            artifact_name = (
                f"m6-b7-l6-hard-witness-positive-gain-coordinate-leaf-{ordinal:03d}.lrat.xz"
            )
            artifact = artifacts / artifact_name
            built, selectors = producer.build_leaf(child)
            producer.write_leaf(cnf, ordinal, child, built, selectors, manifest)
            ...
            records.append((
                f"{ordinal:03d}", producer.child_key(child), f"{child[0]:03d}", child[4][0],
                str(child[1]), str(child[2]), str(child[3]), str(len(child[4][2][6])),
                str(variables), str(clauses), str(cnf_id[0]), cnf_id[1], str(lrat_id[0]),
                lrat_id[1], str(xz_id[0]), xz_id[1], str(solve_ns), str(check_ns),
                f"certificates/{artifact_name}",
            ))
            cnf.unlink()
            lrat.unlink()
            print(
                f"PASS {position:02d}/08 leaf={ordinal:03d} key={producer.child_key(child)} "
        ...
        print(f"PASS leaves=8 total_xz_bytes={total_xz} rows={rows}")
