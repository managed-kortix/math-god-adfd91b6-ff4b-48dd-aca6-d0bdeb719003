...
spectral coloring bounds \cite{AbiadEtAl}.  The general lower bound
$\splus(G)\geq n-1$ is due to Liu--Tang--Zhang \cite{LTZ}.  The present result
settles the $m=n+3$ connected frontier of the edge-surplus conjecture of
Akbari--Kumar--Mohar--Pragada--Zhang \cite{AKMPZ}; it does not address graphs
with more edges by an edge-addition argument.
...
byte-identical where the verifier says so.  The first four commands audit the
new tricyclic theorem used in Lemma~\ref{lem:packets} and in the rank-$3+1$
multiblock rows; the remaining commands audit the rank-four kernel proof.
...
The tricyclic audits reconstruct the finite physical-row ledgers used by the
companion theorem.  Their expected canonical payload digests are, respectively,
\begin{center}\ttfamily\small
...
\end{center}
They are dependencies of the multiblock proof, not part of the seven-verifier
rank-four master manifest.  The census verifier independently enumerates weak edge-multiplicity
...
The exact rank-four dependency manifest printed by the master verifier has
SHA-256 digest
\begin{center}\ttfamily\small
9fa7bdf4a4a296a69f818bf78d5fe1a3aba5bddb38639ea784593d0291dfe19f.
\end{center}
The kernel-census payload itself retains the checksum printed in
Proposition~\ref{prop:kernels}; the manifest digest changed when that verifier
gained automorphism and minimum-simple-subdivision checks and the finite
dependencies gained stricter exact-type checks.  The master now pins seven
direct verifier outputs, eleven transitive dependency files, and three nested
verifier outputs; it also records the historical direct manifest.
...
human authorship, human review, independent verification, or conventional
peer review is claimed.  The exact scripts are corroborative only; the result
should be assessed from the displayed argument and its stated local
dependencies.  The repository post and result card are announcements of this
AI-generated manuscript, not evidence of external validation or publication.
