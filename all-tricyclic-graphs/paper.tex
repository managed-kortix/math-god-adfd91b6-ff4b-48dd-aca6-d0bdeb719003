...
spectral coloring bounds \cite{AbiadEtAl}.  The general $n-1$ lower bound was
proved by Liu--Tang--Zhang \cite{LTZ}; the edge-surplus conjecture motivating
the present statement asks for $\splus(G)\geq n$ when $|E(G)|\geq n+1$
\cite{AKMPZ}.
...
$\splus\geq n$.  The proof establishes $\kappa\leq L+2$ in the DNN rows and
handles the remaining rows by structural repair.
\end{lemma}
...
100&010000&4&DNN\\
101&010001&4&DNN\\
110&010010&8&four DNN, four DNN or deletion\\
111&010011&8&four DNN, four DNN or deletion\\
\bottomrule
\end{tabular}
...
For completeness, the eight rows requiring repair form one orbit under kernel
automorphisms and interchange of parallel-path names; in the fixed switching
normalization, four lie in class $110$ and four in class $111$. Normalize this
orbit so each doubled side has one even and one odd path and the connectors
have opposite parity. If a member of a
...
Here is the independent all-length cover of the last switching class
\cite{K4Cover}.  Its physical parities have the form
$p_{ij}=1\mathbin\oplus s_i\mathbin\oplus s_j$.  After replacing branch vector
$u_i$ by $(-1)^{s_i}u_i$, every transformed endpoint angle is the corresponding
all-odd angle, while the coefficient and denominator remain the physical path
length.  Call a physical path unit if its length is one and long otherwise.
Every even physical path is therefore long.  A nonconstant switch has exactly
three or four even paths, the edges of a nontrivial cut of $K_4$.

With at least three long paths, use four regular-simplex vectors,
$R_{ij}=-1/3$.  A unit path contributes $1/2$, a long odd path contributes
less than $1/6$, and a long even path contributes less than $1/4$.  Four or
more long paths therefore cost less than
$(6-q)/2+q/4\leq2$.  The sole three-long switched configuration has three even
paths of length at least two and three unit odd paths; evaluating the physical
length-two costs more sharply gives less than $3/2+3(1/6)=2$.  If exactly two
paths are long, the switch must be constant, so all paths are physically odd.
There are two placements.  Opposite paths use branch angles
...
attached theta whose three path lengths have the same parity, hence is
bipartite and has credit one; the deleted nonempty tree has credit minus one.
With no long path every switch is constant and the core is $K_4$.  We include
the attached-$K_4$ argument rather than infer it
...
Thus all eight switching classes, including all eight physical rows in the
last class, are covered.
\end{proof}
...
\]
these are the exceptional automorphism orbit (four rows in class $110$ and four
in class $111$) and are closed by the long-path certificates and structural
deletion in the proof of Lemma~\ref{lem:dc4}. Thus the displayed
...
the doubled-$4$-cycle partition into 28 DNN rows and an eight-row exceptional
orbit split equally between classes $110$ and $111$, all 56 physical $K_4$
rows outside the all-odd switching
 class.  The separate all-odd-class audit checks all eight physical switching
 rows and the unit/long lower-length rules.  For the physically all-odd row,
 the finite audit also enumerates all 64 long/unit subsets.  The latter
...
analytic tangent lemma or its all-length coverage.  Its canonical payload also
commits to all audited doubled-$4$-cycle class counts, first and failed rows,
class-$111$ long records, endpoint definitions and tangent constants, together
with the $K_4$ endpoint and arithmetic constants.  Numeric leaves are restricted
to integers and rational strings: floats and all other non-integral numeric
types are rejected explicitly.  The verifier rejects 21 deliberately corrupted
certificate payloads, including formerly uncovered constants.
...
\end{verbatim}
The first two commands must report the certificate digest
\begin{verbatim}
...
README manifest.  Those two commands must also report 21 rejected
hostile mutations.  The standalone $K_4$ audit reports digest
\texttt{85ccff4a03791e2a5a455a7c350b804982f6ee3fb426233cd5e92c67431466c2}
and three rejected mutations.  The standalone doubled-triangle audit reports
digest
\texttt{b293eef0d7742da6ecd2c7af35882be14e97811922493121900e65a6705f01e8}
and three rejected mutations.  The final command audits the Gram obstruction.
More generally, these verifiers do not
...
attached-$K_4$ Sachs argument.  They are fail-closed exact audits of only the
