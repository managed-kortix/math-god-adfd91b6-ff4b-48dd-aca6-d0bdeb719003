...
- 2026-08-06: derived the exact order-eight rank-six frontier. The 325 kernels
  ...
  profiles. Exact search has now certified the first 76000 residual rows (1064000
...
- 2026-07-30: verified the exact rank-twelve sharp-DNN frontier. Exact
  ...
  leave precisely `T^11Q` and `T^10PP`; normal and `-O` runs agree. This is
  only a DNN failure-set classification, not a rank-twelve theorem. The next
  target is an all-rank `T^rPP` proof; finite censuses and special endpoints
  alone are explicitly non-victories. See `positive-square-energy/rank12/prompt.md`.
...
4. Compute structural fingerprints of the certified minimizers.
5. Analyze the flagged unicyclic-with-triangle bottleneck structurally.
6. Use the certified X pipeline only when a result passes every mathematical
